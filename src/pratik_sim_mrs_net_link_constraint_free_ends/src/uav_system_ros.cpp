...
UavSystemRos::UavSystemRos(ros::NodeHandle &nh, const std::string uav_name) {
  ...
  // ROS_INFO("Fifth Hi from UavSystemRos");

  // iterate the model twise to initialize all the states

  uav_system_.makeStep(0.01);
  // ROS_INFO("Sixth Hi from UavSystemRos");
  // ROS_INFO("Bazinga ------- Bazinga -------Bazinga -------Bazinga -------Bazinga -------Bazinga -------");
...
void UavSystemRos::makeStep(const double dt) {
  ...
  // ROS_INFO("First Hi from makestep()");
  ...
  // ROS_INFO(" Third Bazinga ------- Bazinga -------Bazinga -------Bazinga -------Bazinga -------Bazinga -------");

  // publish data
  publishOdometry(state);
  // ROS_INFO(" 3 Bazinga ------- Bazinga -------Bazinga -------Bazinga -------Bazinga -------Bazinga -------");

  // First Rigid Link Only
  publish_quadcopter_state(state);
  // ROS_INFO(" 4 Bazinga ------- Bazinga -------Bazinga -------Bazinga -------Bazinga -------Bazinga -------");

  // First Rigid Link Only
  publish_link_state(state);
  // ROS_INFO(" 5 Bazinga ------- Bazinga -------Bazinga -------Bazinga -------Bazinga -------Bazinga -------");

  // First Rigid Link Only
  // publish_pos_of_link_load(state);
  // ROS_INFO(" 6 Bazinga ------- Bazinga -------Bazinga -------Bazinga -------Bazinga -------Bazinga -------");

  // First Rigid Link Only
  // publish_pos_of_link_load_in_rviz(state);
  // ROS_INFO(" 7 Bazinga ------- Bazinga -------Bazinga -------Bazinga -------Bazinga -------Bazinga -------");

  // Publish all the points masses
  publish_pos_of_link_loads_in_rviz(state);
  // ROS_INFO(" 8 Bazinga ------- Bazinga -------Bazinga -------Bazinga -------Bazinga -------Bazinga -------");

  publishIMU(state);
  // ROS_INFO(" 9 Bazinga ------- Bazinga -------Bazinga -------Bazinga -------Bazinga -------Bazinga -------");

  publishRangefinder(state);
  // ROS_INFO(" 10 Bazinga ------- Bazinga -------Bazinga -------Bazinga -------Bazinga -------Bazinga -------");
...
void UavSystemRos::publish_link_state(const MultirotorModel::State &state) {
...
void UavSystemRos::publish_pos_of_link_loads_in_rviz(const MultirotorModel::State &state) {
  ...
  for (int i = 0; i < NO_OF_CAP; ++i) {
    for (int j = 0; j < NO_OF_LINKS; ++j) {
      ...
      for ( int a = 1; a <= j; ++a ){
        sum_a_1_j_l_ia_q_ia = sum_a_1_j_l_ia_q_ia + link_lengths_array(i, j) * q_each_link_local[i][j];
      }

      Eigen::Vector3d qb_vec_temp (-std::sin(state.alpha), 0.0, -std::cos(state.alpha));

      Eigen::Vector3d pos_of_load   = state.x + state.R * ( rho_vector + rho_i_vectors[i] + l_first_link * qb_vec_temp) + sum_a_1_j_l_ia_q_ia;
