...
#include <pratik_controller_mrs_net_control_link_only/controller_mrs_net_control_link_onlyConfig.h>
...
namespace pratik_controller_mrs_net_control_link_only
{

namespace controller_mrs_net_control_link_only
{
...
/* //{ class ControllerMrsNetControlLinkOnly */

class ControllerMrsNetControlLinkOnly : public mrs_uav_managers::Controller {
...
private:
  ...
  typedef pratik_controller_mrs_net_control_link_only::controller_mrs_net_control_link_onlyConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>            Drs_t;
  boost::shared_ptr<Drs_t>                                    drs_;
  void                                                        callbackDrs(pratik_controller_mrs_net_control_link_only::controller_mrs_net_control_link_onlyConfig& config, uint32_t level);
...
bool ControllerMrsNetControlLinkOnly::initialize(const ros::NodeHandle& nh, std::shared_ptr<mrs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                                   std::shared_ptr<mrs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  success *= private_handlers->loadConfigFile(ros::package::getPath("pratik_controller_mrs_net_control_link_only") + "/config/controller_mrs_net_control_link_only.yaml");

  if (!success) {
    return false;
  }

  mrs_lib::ParamLoader param_loader(nh_, "ControllerMrsNetControlLinkOnly");

  const std::string yaml_namespace = "pratik_controller_mrs_net_control_link_only/default_gains/";
  ...
  // gain filtering
  const std::string yaml_namespace_gain = "pratik_controller_mrs_net_control_link_only/gain_filtering/";
  ...
  const std::string yaml_namespace_ang_rate_ff = "pratik_controller_mrs_net_control_link_only/angular_rate_feedforward/";
  ...
  // constraints
  private_handlers->param_loader->loadParam("pratik_controller_mrs_net_control_link_only/constraints/tilt_angle_failsafe/enabled", _tilt_angle_failsafe_enabled_);
  private_handlers->param_loader->loadParam("pratik_controller_mrs_net_control_link_only/constraints/tilt_angle_failsafe/limit", _tilt_angle_failsafe_);
  ...
  if (_tilt_angle_failsafe_enabled_ && std::abs(_tilt_angle_failsafe_) < 1e-3) {
    ROS_ERROR("[ControllerMrsNetControlLinkOnly]: constraints/tilt_angle_failsafe/enabled = 'TRUE' but the limit is too low");
    return false;
  }
  ...
  if (!param_loader.loadedSuccessfully()) {
    ROS_ERROR("[ControllerMrsNetControlLinkOnly]: could not load all parameters!");
    return false;
  }
  ...
  Drs_t::CallbackType f = boost::bind(&ControllerMrsNetControlLinkOnly::callbackDrs, this, _1, _2);
  drs_->setCallback(f);

  // | ------------------------- timers ------------------------- |

  timer_gains_ = nh_.createTimer(ros::Rate(_gain_filtering_rate_), &ControllerMrsNetControlLinkOnly::timerGains, this, false, false);

  // | ------------------ initialize subscribers ----------------- |

  mrs_lib::SubscribeHandlerOptions shopts;
  shopts.nh                 = nh;
  shopts.node_name          = "ControllerMrsNetControlLinkOnly";
  ...
  sh_link_states           = mrs_lib::SubscribeHandler<nav_msgs::Odometry>(shopts, "/sim_mrs_net_control_link_only/uav1/link_state",
                                                                                            &ControllerMrsNetControlLinkOnly::callback_link_states, this);
  ...
  ROS_INFO("[ControllerMrsNetControlLinkOnly]: initialized");
...
bool ControllerMrsNetControlLinkOnly::activate(const ControlOutput& last_control_output) {
  ...
  if (activation_control_output_.diagnostics.mass_estimator) {
    ...
    ROS_INFO("[ControllerMrsNetControlLinkOnly]: setting mass difference from the last control output: %.2f kg", uav_mass_difference_);
  }
  ...
  ROS_INFO("[ControllerMrsNetControlLinkOnly]: activated");
...
void ControllerMrsNetControlLinkOnly::deactivate(void) {
  ...
  ROS_INFO("[ControllerMrsNetControlLinkOnly]: deactivated");
}
...
void ControllerMrsNetControlLinkOnly::updateInactive(const mrs_msgs::UavState& uav_state, [[maybe_unused]] const std::optional<mrs_msgs::TrackerCommand>& tracker_command) {
...
ControllerMrsNetControlLinkOnly::ControlOutput ControllerMrsNetControlLinkOnly::updateActive(const mrs_msgs::UavState& uav_state, const mrs_msgs::TrackerCommand& tracker_command) {
  ...
  if (fabs(dt) < 0.001) {
    ...
    ROS_DEBUG("[ControllerMrsNetControlLinkOnly]: the last odometry message came too close (%.2f s)!", dt);
    dt = 0.01;
    // dt               = 0.004;
  }
  ...
  if (common_handlers_->control_output_modalities.attitude) {
    ROS_INFO_THROTTLE(1.0, "[ControllerMrsNetControlLinkOnly]: desired attitude output modality is available");
  }
  ...
  if (common_handlers_->detailed_model_params) {
    ...
    // ROS_INFO_STREAM_THROTTLE(1.0, "[ControllerMrsNetControlLinkOnly]: UAV inertia is: " << detailed_model_params.inertia);
  }
  ...
  if (u_control_input(2) < 0) {
    ROS_WARN_THROTTLE(1.0, "[ControllerMrsNetControlLinkOnly]: the calculated downwards desired force is negative (%.2f) -> mitigating flip", u_control_input(2));
  ...
  if (tracker_command.use_orientation) {
    ...
    if (tracker_command.use_heading) {
      try {
        ...
        ROS_ERROR_THROTTLE(1.0, "[ControllerMrsNetControlLinkOnly]: could not set the desired heading");
      }
    }
    ...
    if (tracker_command.use_heading) {
      ...
      ROS_WARN_THROTTLE(10.0, "[ControllerMrsNetControlLinkOnly]: desired heading was not specified, using current heading instead!");
  ...
  else
  {
    if (desired_thrust_force >= 0) {
      ...
      ROS_WARN_THROTTLE(1.0, "[ControllerMrsNetControlLinkOnly]: just so you know, the desired throttle force is negative (%.2f)", desired_thrust_force);
    }
  }
  ...
  last_control_output_.diagnostics.controller       = "ControllerMrsNetControlLinkOnly";
...
Eigen::Vector3d ControllerMrsNetControlLinkOnly::orientationError(const Eigen::Matrix3d& R, const Eigen::Matrix3d& Rd) {
...
const mrs_msgs::ControllerStatus ControllerMrsNetControlLinkOnly::getStatus() {
...
void ControllerMrsNetControlLinkOnly::switchOdometrySource([[maybe_unused]] const mrs_msgs::UavState& new_uav_state) {
}
...
void ControllerMrsNetControlLinkOnly::resetDisturbanceEstimators(void) {
}
...
const mrs_msgs::DynamicsConstraintsSrvResponse::ConstPtr ControllerMrsNetControlLinkOnly::setConstraints([
  ...
  ROS_INFO("[ControllerMrsNetControlLinkOnly]: updating constraints");
...
void ControllerMrsNetControlLinkOnly::callbackDrs(pratik_controller_mrs_net_control_link_only::controller_mrs_net_control_link_onlyConfig& config, [[maybe_unused]] uint32_t level) {

  mrs_lib::set_mutexed(mutex_drs_params_, config, drs_params_);

  ROS_INFO("[ControllerMrsNetControlLinkOnly]: dynamic reconfigure params updated");
}
...
void ControllerMrsNetControlLinkOnly::timerGains(const ros::TimerEvent& event) {

  mrs_lib::Routine    profiler_routine = profiler_.createRoutine("timerGains", _gain_filtering_rate_, 1.0, event);
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer("ControllerMrsNetControlLinkOnly::timerGains", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
  ...
  if (updated) {
    ...
    ROS_INFO_THROTTLE(10.0, "[ControllerMrsNetControlLinkOnly]: gains have been updated");
  }
}
...
std::optional<mrs_msgs::HwApiAttitudeRateCmd> ControllerMrsNetControlLinkOnly::attitudeController(const mrs_msgs::UavState& uav_state, const mrs_msgs::HwApiAttitudeCmd& reference,
                                                                 const Eigen::Vector3d& ff_rate, const Eigen::Vector3d& rate_saturation,
                                                                 const Eigen::Vector3d& gains, const bool& parasitic_heading_rate_compensation) {
...
void ControllerMrsNetControlLinkOnly::callback_link_states(const nav_msgs::Odometry::ConstPtr msg) {

  alpha     = msg->pose.pose.position.x;
  alpha_dot = msg->twist.twist.linear.x;
}

float ControllerMrsNetControlLinkOnly::clipping_angle(float max_value, float current_angle){
...
float ControllerMrsNetControlLinkOnly::clipping_net_thrust_force(float max_value, float current_thrust){
...
Eigen::Vector3d ControllerMrsNetControlLinkOnly::clipping_e_x_q(Eigen::Vector3d e_x_q_vector){
...
Eigen::Vector3d ControllerMrsNetControlLinkOnly::clipping_e_x_q_dot(Eigen::Vector3d e_x_q_dot_vector){
...
float ControllerMrsNetControlLinkOnly::distance_bt_two_pts(Eigen::Vector3d A, Eigen::Vector3d B){
...
Eigen::Vector3d ControllerMrsNetControlLinkOnly::Matrix_vector_mul(Eigen::Matrix3d R, Eigen::Vector3d v){
  Eigen::Vector3d mul_vector (R(0,0)*v[0] + R(0,1)*v[1] + R(0,2)*v[2], R(1,0)*v[0] + R(1,1)*v[1] + R(1,2)*v[2],  R(2,0)*v[0] + R(2,1)*v[1] + R(2,2)*v[2]);
  return mul_vector;
}

Eigen::Vector3d ControllerMrsNetControlLinkOnly::Rotation_matrix_to_Euler_angle(Eigen::Matrix3d R){
...
Eigen::Matrix3d ControllerMrsNetControlLinkOnly::hatmap(const Eigen::Vector3d &v) {
  Eigen::Matrix3d H;
  H <<     0.0, -v.z(),  v.y(),
        v.z(),    0.0, -v.x(),
       -v.y(),  v.x(),    0.0;
...
Eigen::Matrix3d ControllerMrsNetControlLinkOnly::so3transform(const Eigen::Vector3d& body_z, const ::Eigen::Vector3d& heading, const bool& preserve_heading) {
...
double ControllerMrsNetControlLinkOnly::calculateGainChange(const double dt, const double current_value, const double desired_value, const bool bypass_rate, std::string name,
                                          bool& updated) {
  ...
  if (std::abs(change) > 1e-3) {
    ROS_DEBUG("[ControllerMrsNetControlLinkOnly]: changing gain '%s' from %.2f to %.2f", name.c_str(), current_value, desired_value);
    updated = true;
  }
...
double ControllerMrsNetControlLinkOnly::getHeadingSafely(const mrs_msgs::UavState& uav_state, const mrs_msgs::TrackerCommand& tracker_command) {
...
// void ControllerMrsNetControlLinkOnly::Publisher_QuadState(const mrs_msgs::UavState& uav_state){
...
// void ControllerMrsNetControlLinkOnly::timerPublishQuadState([[maybe_unused]] const ros::TimerEvent& te, const mrs_msgs::UavState& uav_state) {
...
PLUGINLIB_EXPORT_CLASS(pratik_controller_mrs_net_control_link_only::controller_mrs_net_control_link_only::ControllerMrsNetControlLinkOnly, mrs_uav_managers::Controller)
