...
#include <pratik_sim_one_drone/sim_one_droneConfig.h>
...
namespace pratik_sim_one_drone
{

typedef std::vector<Eigen::VectorXd> my_vector_of_vectors_t;

/* class PratikSimOneDrone //{ */

class PratikSimOneDrone : public nodelet::Nodelet {
...
private:
  ...
  typedef pratik_sim_one_drone::sim_one_droneConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>             Drs_t;
  boost::shared_ptr<Drs_t>                                     drs_;
  void                                                         callbackDrs(pratik_sim_one_drone::sim_one_droneConfig& config, uint32_t level);
...
void PratikSimOneDrone::onInit() {
  ...
  mrs_lib::ParamLoader param_loader(nh_, "PratikSimOneDrone");
  ...
  for (size_t i = 0; i < uav_names.size(); i++) {
    ...
    ROS_INFO("[PratikSimOneDrone]: initializing '%s'", uav_name.c_str());
  ...
  Drs_t::CallbackType f = boost::bind(&PratikSimOneDrone::callbackDrs, this, _1, _2);
  drs_->setCallback(f);

  if (!param_loader.loadedSuccessfully()) {
    ROS_ERROR("[PratikSimOneDrone]: could not load all parameters!");
    ros::shutdown();
  }
  ...
  timer_main_ = nh_.createWallTimer(ros::WallDuration(1.0 / (_simulation_rate_ * drs_params_.realtime_factor)), &PratikSimOneDrone::timerMain, this);

  timer_status_ = nh_.createWallTimer(ros::WallDuration(1.0), &PratikSimOneDrone::timerStatus, this);

  // | ----------------------- finish init ---------------------- |

  is_initialized_ = true;

  ROS_INFO("[PratikSimOneDrone]: initialized");
}
...
void PratikSimOneDrone::timerMain([[maybe_unused]] const ros::WallTimerEvent& event) {

  if (!is_initialized_) {
    return;
  }

  ROS_INFO_ONCE("[PratikSimOneDrone]: main timer spinning");
...
void PratikSimOneDrone::timerStatus([[maybe_unused]] const ros::WallTimerEvent& event) {
  ...
  ROS_INFO_THROTTLE(0.1, "[PratikSimOneDrone]: %s, desired RTF = %.2f, actual RTF = %.2f", drs_params.paused ? "paused" : "running",
...
void PratikSimOneDrone::callbackDrs(pratik_sim_one_drone::sim_one_droneConfig& config, [[maybe_unused]] uint32_t level) {
  ...
  ROS_INFO("[PratikSimOneDrone]: DRS updated params");
}
...
void PratikSimOneDrone::handleCollisions(void) {
...
void PratikSimOneDrone::publishPoses(void) {
...
PLUGINLIB_EXPORT_CLASS(pratik_sim_one_drone::PratikSimOneDrone, nodelet::Nodelet)
