...
#include <mrs_uav_controllers_cac/Se3Controller_CACConfig.h>
...
// Eigen::Array3d kq(5.0,5.0,5.0);
// Eigen::Array3d kq_dot(4.5,4.5,4.5);
...
namespace mrs_uav_controllers_cac
{

namespace se3_controller_CAC
{
...
typedef struct
{
  double kqcac_xy;       // pendulum (CAC) xy gain
  double kqcac_z;        // pendulum (CAC) z gain
  double kqcac_dot_xy;   // pendulum (CAC) dot xy gain
  double kqcac_dot_z;    // pendulum (CAC) dot z gain
...
/* //{ class Se3Controller_CAC */

class Se3Controller_CAC : public mrs_uav_managers::Controller {
...
private:
  ...
  typedef mrs_uav_controllers_cac::Se3Controller_CACConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>  Drs_t;
  boost::shared_ptr<Drs_t>                          drs_;
  void                                              callbackDrs(mrs_uav_controllers_cac::Se3Controller_CACConfig& config, uint32_t level);
...
bool Se3Controller_CAC::initialize(const ros::NodeHandle& nh, std::shared_ptr<mrs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                               std::shared_ptr<mrs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  private_handlers->param_loader->addYamlFile(ros::package::getPath("mrs_uav_controllers_cac") + "/config/private/se3_controller_CAC.yaml");
  private_handlers->param_loader->addYamlFile(ros::package::getPath("mrs_uav_controllers_cac") + "/config/public/se3_controller_CAC.yaml");

  const std::string yaml_namespace = "mrs_uav_controllers_cac/se3_controller_CAC/";

  // pendulum (CAC) gains
  private_handlers->param_loader->loadParam(yaml_namespace + "se3/default_gains/pendulum/kqcac_xy", gains_.kqcac_xy);
  private_handlers->param_loader->loadParam(yaml_namespace + "se3/default_gains/pendulum/kqcac_z", gains_.kqcac_z);
  private_handlers->param_loader->loadParam(yaml_namespace + "se3/default_gains/pendulum/kqcac_dot_xy", gains_.kqcac_dot_xy);
  private_handlers->param_loader->loadParam(yaml_namespace + "se3/default_gains/pendulum/kqcac_dot_z", gains_.kqcac_dot_z);
  ...
  if (_tilt_angle_failsafe_enabled_ && std::abs(_tilt_angle_failsafe_) < 1e-3) {
    ROS_ERROR("[Se3Controller_CAC]: constraints/tilt_angle_failsafe/enabled = 'TRUE' but the limit is too low");
    return false;
  }
  ...
  if (!private_handlers->param_loader->loadedSuccessfully()) {
    ROS_ERROR("[Se3Controller_CAC]: could not load all parameters!");
    return false;
  }
  ...
  if (!(drs_params_.preferred_output_mode == OUTPUT_ACTUATORS || drs_params_.preferred_output_mode == OUTPUT_CONTROL_GROUP ||
    ...
    ROS_ERROR("[Se3Controller_CAC]: preferred output mode has to be {0, 1, 2, 3}!");
    return false;
  }
  ...
  drs_params_.kqcac_xy         = gains_.kqcac_xy;
  drs_params_.kqcac_z          = gains_.kqcac_z;
  drs_params_.kqcac_dot_xy     = gains_.kqcac_dot_xy;
  drs_params_.kqcac_dot_z      = gains_.kqcac_dot_z;
  ...
  Drs_t::CallbackType f = boost::bind(&Se3Controller_CAC::callbackDrs, this, _1, _2);
  drs_->setCallback(f);

  // | ------------------------- timers ------------------------- |

  timer_gains_ = nh_.createTimer(ros::Rate(_gain_filtering_rate_), &Se3Controller_CAC::timerGains, this, false, false);
  ...
  profiler_ = mrs_lib::Profiler(common_handlers_->parent_nh, "Se3Controller_CAC", _profiler_enabled_);

  // callback
  mrs_lib::SubscribeHandlerOptions shopts;
  shopts.nh                 = nh;
  shopts.node_name          = "se3_controller_CAC";
  ...
  std::string topic_name_Cable_states = "/" + uav_name + "/cable_state";

  sh_cable_states           = mrs_lib::SubscribeHandler<nav_msgs::Odometry>(shopts, topic_name_Cable_states,
                                                                                            &Se3Controller_CAC::callback_cable_states, this);


  // | ----------------------- finish init ---------------------- |

  ROS_INFO("[Se3Controller_CAC]: initialized");
...
bool Se3Controller_CAC::activate(const ControlOutput& last_control_output) {
  ...
  if (activation_control_output_.diagnostics.mass_estimator) {
    ...
    ROS_INFO("[Se3Controller_CAC]: setting mass difference from the last control output: %.2f kg", uav_mass_difference_);
  }
  ...
  if (activation_control_output_.diagnostics.disturbance_estimator) {
    ...
    ROS_INFO(
        "[Se3Controller_CAC]: setting disturbances from the last control output: Ib_b_: %.2f, %.2f N, Iw_w_: "
        "%.2f, %.2f N",
  ...
  if (_rampup_enabled_ && throttle_last_controller) {
    ...
    ROS_INFO("[Se3Controller_CAC]: activating rampup with initial throttle: %.4f, target: %.4f", throttle_last_controller.value(), hover_throttle);
  ...
  ROS_INFO("[Se3Controller_CAC]: activated");
...
void Se3Controller_CAC::deactivate(void) {
  ...
  ROS_INFO("[Se3Controller_CAC]: deactivated");
}
...
void Se3Controller_CAC::updateInactive(const mrs_msgs::UavState& uav_state, [[maybe_unused]] const std::optional<mrs_msgs::TrackerCommand>& tracker_command) {
...
Se3Controller_CAC::ControlOutput Se3Controller_CAC::updateActive(const mrs_msgs::UavState& uav_state, const mrs_msgs::TrackerCommand& tracker_command) {

  mrs_lib::Routine    profiler_routine = profiler_.createRoutine("updateActive");
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer("Se3Controller_CAC::updateActive", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
  ...
  if (std::abs(dt) < 0.001) {
    ...
    ROS_DEBUG("[Se3Controller_CAC]: the last odometry message came too close (%.2f s)!", dt);
  ...
  if (!lowest_modality) {
    ...
    ROS_ERROR_THROTTLE(1.0, "[Se3Controller_CAC]: output modalities are empty! This error should never appear.");
  ...
  if (drs_params.preferred_output_mode == OUTPUT_ATTITUDE_RATE && common_handlers_->control_output_modalities.attitude_rate) {
    ROS_DEBUG_THROTTLE(1.0, "[Se3Controller_CAC]: prioritizing attitude rate output");
    ...
    ROS_DEBUG_THROTTLE(1.0, "[Se3Controller_CAC]: prioritizing attitude output");
    ...
    ROS_DEBUG_THROTTLE(1.0, "[Se3Controller_CAC]: prioritizing control group output");
    ...
    ROS_DEBUG_THROTTLE(1.0, "[Se3Controller_CAC]: prioritizing actuators output");
...
const mrs_msgs::ControllerStatus Se3Controller_CAC::getStatus() {
...
void Se3Controller_CAC::switchOdometrySource(const mrs_msgs::UavState& new_uav_state) {

  ROS_INFO("[Se3Controller_CAC]: switching the odometry source");
  ...
  if (res) {
    ...
    ROS_ERROR_THROTTLE(1.0, "[Se3Controller_CAC]: could not transform world integral to the new frame");
...
void Se3Controller_CAC::resetDisturbanceEstimators(void) {
...
const mrs_msgs::DynamicsConstraintsSrvResponse::ConstPtr Se3Controller_CAC::setConstraints([
  ...
  ROS_INFO("[Se3Controller_CAC]: updating constraints");
...
void Se3Controller_CAC::SE3Controller(const mrs_msgs::UavState& uav_state, const mrs_msgs::TrackerCommand& tracker_command, const double& dt,
                                  const common::CONTROL_OUTPUT& output_modality) {
...

  {
    ...
    if (res) {
      ...
      ROS_ERROR_THROTTLE(1.0, "[Se3Controller_CAC]: could not transform the Ib_b_ to the world frame");
    }
  }
...

  {
    ...
    if (!std::isfinite(Iw_w_(0))) {
      Iw_w_(0) = 0;
      ROS_ERROR_THROTTLE(1.0, "[Se3Controller_CAC]: NaN detected in variable 'Iw_w_(0)', setting it to 0!!!");
    ...
    if (gains.kiwxy_lim >= 0 && world_integral_saturated) {
      ROS_WARN_THROTTLE(1.0, "[Se3Controller_CAC]: SE3's world X integral is being saturated!");
    }
    ...
    if (!std::isfinite(Iw_w_(1))) {
      Iw_w_(1) = 0;
      ROS_ERROR_THROTTLE(1.0, "[Se3Controller_CAC]: NaN detected in variable 'Iw_w_(1)', setting it to 0!!!");
    ...
    if (gains.kiwxy_lim >= 0 && world_integral_saturated) {
      ROS_WARN_THROTTLE(1.0, "[Se3Controller_CAC]: SE3's world Y integral is being saturated!");
    }
  }
...

  {
    ...
    // get the position control error in the fcu_untilted frame
    {
      ...
      if (res) {
        ...
        ROS_ERROR_THROTTLE(1.0, "[Se3Controller_CAC]: could not transform the position error to fcu_untilted");
      }
    }
    ...
    // get the velocity control error in the fcu_untilted frame
    {
      ...
      if (res) {
        ...
        ROS_ERROR_THROTTLE(1.0, "[Se3Controller_CAC]: could not transform the velocity error to fcu_untilted");
      }
    }
    ...
    if (!std::isfinite(Ib_b_(0))) {
      Ib_b_(0) = 0;
      ROS_ERROR_THROTTLE(1.0, "[Se3Controller_CAC]: NaN detected in variable 'Ib_b_(0)', setting it to 0!!!");
    ...
    if (gains.kibxy_lim > 0 && body_integral_saturated) {
      ROS_WARN_THROTTLE(1.0, "[Se3Controller_CAC]: SE3's body pitch integral is being saturated!");
    }
    ...
    if (!std::isfinite(Ib_b_(1))) {
      Ib_b_(1) = 0;
      ROS_ERROR_THROTTLE(1.0, "[Se3Controller_CAC]: NaN detected in variable 'Ib_b_(1)', setting it to 0!!!");
    ...
    if (gains.kibxy_lim > 0 && body_integral_saturated) {
      ROS_WARN_THROTTLE(1.0, "[Se3Controller_CAC]: SE3's body roll integral is being saturated!");
    }
  }
  ...
  if (output_modality == common::ACCELERATION_HDG || output_modality == common::ACCELERATION_HDG_RATE) {
    ...
    last_control_output_.diagnostics.controller = "Se3Controller_CAC";
...

  {
    ...
    if (!std::isfinite(uav_mass_difference_)) {
      uav_mass_difference_ = 0;
      ROS_WARN_THROTTLE(1.0, "[Se3Controller_CAC]: NaN detected in variable 'uav_mass_difference_', setting it to 0 and returning!!!");
    ...
    if (uav_mass_saturated) {
      ROS_WARN_THROTTLE(1.0, "[Se3Controller_CAC]: The UAV mass difference is being saturated to %.2f!", uav_mass_difference_);
    }
  }
  ...
  const Eigen::Array3d kqcac(gains.kqcac_xy, gains.kqcac_xy, gains.kqcac_z);
  const Eigen::Array3d kqcac_dot(gains.kqcac_dot_xy, gains.kqcac_dot_xy, gains.kqcac_dot_z);

  ROS_INFO_STREAM_THROTTLE(1.0, "kqcac: " << kqcac);
  ROS_INFO_STREAM_THROTTLE(1.0, "kqcac_dot: " << kqcac_dot);

  // | ---------------- prepare the cable attitude control output --------------- |
  e_q               = q.cross(q.cross(q_d));
  e_q_dot           = q_dot - (q_d.cross(q_d_dot)).cross(q);
  u_cable_input     = kqcac * e_q.array()  + kqcac_dot * e_q_dot.array();

  f = f + u_cable_input;

  // ROS_INFO_THROTTLE(1.0, "I am from downloaded SE3 Controller");
  ...
  if (f(2) < 0) {
    ...
    ROS_WARN_THROTTLE(1.0, "[Se3Controller_CAC]: the calculated downwards desired force is negative (%.2f) -> mitigating flip", f(2));
  ...
  auto f_normed_sanitized = common::sanitizeDesiredForce(f.normalized(), tilt_safety_limit, constraints.tilt, "Se3Controller_CAC");

  if (!f_normed_sanitized) {

    ROS_INFO("[Se3Controller_CAC]: position feedback: [%.2f, %.2f, %.2f]", position_feedback(0), position_feedback(1), position_feedback(2));
    ROS_INFO("[Se3Controller_CAC]: velocity feedback: [%.2f, %.2f, %.2f]", velocity_feedback(0), velocity_feedback(1), velocity_feedback(2));
    ROS_INFO("[Se3Controller_CAC]: integral feedback: [%.2f, %.2f, %.2f]", integral_feedback(0), integral_feedback(1), integral_feedback(2));
    ROS_INFO("[Se3Controller_CAC]: tracker_cmd: x: %.2f, y: %.2f, z: %.2f, heading: %.2f", tracker_command.position.x, tracker_command.position.y,
             tracker_command.position.z, tracker_command.heading);
    ROS_INFO("[Se3Controller_CAC]: odometry: x: %.2f, y: %.2f, z: %.2f, heading: %.2f", uav_state.pose.position.x, uav_state.pose.position.y,
  ...
  if (tracker_command.use_orientation) {
    ...
    if (tracker_command.use_heading) {
      try {
        ...
        ROS_ERROR_THROTTLE(1.0, "[Se3Controller_CAC]: could not set the desired heading");
      }
    }
    ...
    if (tracker_command.use_heading) {
      ...
      ROS_WARN_THROTTLE(10.0, "[Se3Controller_CAC]: desired heading was not specified, using current heading instead!");
  ...
  if (tracker_command.use_throttle) {
    ...
    if (std::abs((ros::Time::now() - rampup_start_time_).toSec()) >= rampup_duration_) {
      ...
      ROS_INFO("[Se3Controller_CAC]: rampup finished");
      ...
      ROS_INFO_THROTTLE(0.1, "[Se3Controller_CAC]: ramping up throttle, %.4f", rampup_throttle_);
    }
    ...
    if (desired_thrust_force >= 0) {
      ...
      ROS_WARN_THROTTLE(1.0, "[Se3Controller_CAC]: just so you know, the desired throttle force is negative (%.2f)", desired_thrust_force);
    }
  }
  ...
  if (!std::isfinite(throttle)) {
    ...
    ROS_ERROR("[Se3Controller_CAC]: NaN detected in variable 'throttle'!!!");
    return;

  } else if (throttle > _throttle_saturation_) {
    throttle = _throttle_saturation_;
    ROS_WARN_THROTTLE(0.1, "[Se3Controller_CAC]: saturating throttle to %.2f", _throttle_saturation_);
  } else if (throttle < 0.0) {
    throttle = 0.0;
    ROS_WARN_THROTTLE(0.1, "[Se3Controller_CAC]: saturating throttle to 0.0");
  }

  if (throttle_saturated) {
    ROS_WARN_THROTTLE(0.1, "[Se3Controller_CAC]: ---------------------------");
    ROS_WARN_THROTTLE(0.1, "[Se3Controller_CAC]: desired state: pos [x: %.2f, y: %.2f, z: %.2f, hdg: %.2f]", tracker_command.position.x, tracker_command.position.y,
                      tracker_command.position.z, tracker_command.heading);
    ROS_WARN_THROTTLE(0.1, "[Se3Controller_CAC]: desired state: vel [x: %.2f, y: %.2f, z: %.2f, hdg: %.2f]", tracker_command.velocity.x, tracker_command.velocity.y,
                      tracker_command.velocity.z, tracker_command.heading_rate);
    ROS_WARN_THROTTLE(0.1, "[Se3Controller_CAC]: desired state: acc [x: %.2f, y: %.2f, z: %.2f, hdg: %.2f]", tracker_command.acceleration.x,
                      tracker_command.acceleration.y, tracker_command.acceleration.z, tracker_command.heading_acceleration);
    ROS_WARN_THROTTLE(0.1, "[Se3Controller_CAC]: desired state: jerk [x: %.2f, y: %.2f, z: %.2f, hdg: %.2f]", tracker_command.jerk.x, tracker_command.jerk.y,
                      tracker_command.jerk.z, tracker_command.heading_jerk);
    ROS_WARN_THROTTLE(0.1, "[Se3Controller_CAC]: ---------------------------");
    ROS_WARN_THROTTLE(0.1, "[Se3Controller_CAC]: current state: pos [x: %.2f, y: %.2f, z: %.2f, hdg: %.2f]", uav_state.pose.position.x, uav_state.pose.position.y,
                      uav_state.pose.position.z, uav_heading);
    ROS_WARN_THROTTLE(0.1, "[Se3Controller_CAC]: current state: vel [x: %.2f, y: %.2f, z: %.2f, yaw rate: %.2f]", uav_state.velocity.linear.x,
  ...
  if (tracker_command.use_attitude_rate) {
    ...
    try {
      ...
      ROS_ERROR("[Se3Controller_CAC]: exception caught while calculating the desired_yaw_rate feedforward");
    }
  ...
  if (tracker_command.use_jerk && drs_params.jerk_feedforward) {
    ...
    ROS_DEBUG_THROTTLE(1.0, "[Se3Controller_CAC]: using jerk feedforward");
...
void Se3Controller_CAC::positionPassthrough(const mrs_msgs::UavState& uav_state, const mrs_msgs::TrackerCommand& tracker_command) {

  if (!tracker_command.use_position_vertical || !tracker_command.use_position_horizontal || !tracker_command.use_heading) {
    ROS_ERROR("[Se3Controller_CAC]: the tracker did not provide position+hdg reference");
    return;
  }
...
void Se3Controller_CAC::PIDVelocityOutput(const mrs_msgs::UavState& uav_state, const mrs_msgs::TrackerCommand& tracker_command,
  ...
  if (control_output == common::VELOCITY_HDG) {
    ...
    ROS_ERROR("[Se3Controller_CAC]: the required output of the position PID is not supported");
    return;
  }
...
void Se3Controller_CAC::callbackDrs(mrs_uav_controllers_cac::Se3Controller_CACConfig& config, [[maybe_unused]] uint32_t level) {

  mrs_lib::set_mutexed(mutex_drs_params_, config, drs_params_);

  ROS_INFO("[Se3Controller_CAC]: DRS updated gains");
}
...
void Se3Controller_CAC::timerGains(const ros::TimerEvent& event) {

  mrs_lib::Routine    profiler_routine = profiler_.createRoutine("timerGains", _gain_filtering_rate_, 1.0, event);
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer("Se3Controller_CAC::timerGains", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
  ...
  gains.kqcac_xy      = calculateGainChange(dt, gains.kqcac_xy, drs_params.kqcac_xy * gain_coeff, bypass_filter, "kqcac_xy", updated);
  gains.kqcac_z       = calculateGainChange(dt, gains.kqcac_z,  drs_params.kqcac_z * gain_coeff, bypass_filter, "kqcac_z", updated);
  gains.kqcac_dot_xy  = calculateGainChange(dt, gains.kqcac_dot_xy, drs_params.kqcac_dot_xy * gain_coeff, bypass_filter, "kqcac_dot_xy", updated);
  gains.kqcac_dot_z   = calculateGainChange(dt, gains.kqcac_dot_z,  drs_params.kqcac_dot_z * gain_coeff, bypass_filter, "kqcac_dot_z", updated);
  ...
  if (updated) {
    ...
    drs_params.kqcac_xy      = gains.kqcac_xy;
    drs_params.kqcac_z       = gains.kqcac_z;
    drs_params.kqcac_dot_xy  = gains.kqcac_dot_xy;
    drs_params.kqcac_dot_z   = gains.kqcac_dot_z;
    ...
    ROS_INFO_THROTTLE(10.0, "[Se3Controller_CAC]: gains have been updated");
  }
}
...
double Se3Controller_CAC::calculateGainChange(const double dt, const double current_value, const double desired_value, const bool bypass_rate, std::string name,
                                          bool& updated) {
  ...
  if (std::abs(change) > 1e-3) {
    ROS_DEBUG("[Se3Controller_CAC]: changing gain '%s' from %.2f to %.2f", name.c_str(), current_value, desired_value);
    updated = true;
  }
...
double Se3Controller_CAC::getHeadingSafely(const mrs_msgs::UavState& uav_state, const mrs_msgs::TrackerCommand& tracker_command) {
...
void Se3Controller_CAC::callback_cable_states(const nav_msgs::Odometry::ConstPtr msg) {
...
PLUGINLIB_EXPORT_CLASS(mrs_uav_controllers_cac::se3_controller_CAC::Se3Controller_CAC, mrs_uav_managers::Controller)
