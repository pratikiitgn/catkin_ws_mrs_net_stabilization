...
PROJECT_NAME = Final_Exp_Pratik_Single_Drone
...
'
  'Core' 'waitForTime; roslaunch mrs_uav_core core.launch platform_config:=`rospack find mrs_uav_deployment`/config/mrs_uav_system/$UAV_TYPE.yaml world_config:=./config/world_temesvar_field.yaml custom_config:=./config/custom_config.yaml network_config:=./config/network_config.yaml
'
  'AutoStart' 'waitForHw; roslaunch mrs_uav_autostart automatic_start.launch
'
  'CAM' 'waitForHw; roslaunch mrs_llcp_ros llcp.launch
'
  'trajectory_load' 'waitForTakeoff; roslaunch mrs_uav_trajectory_loader load.launch config:=./config/trajectory_loader.yaml path:=`rospack find mrs_uav_controllers_cac`/trajectories
'  
  'trajectory_goto_start' 'history -s roslaunch mrs_uav_trajectory_loader goto_start.launch config:=./config/trajectory_loader.yaml path:=`rospack find mrs_uav_controllers_cac`/trajectories
' 
  'trajectory_start_tracking' 'history -s roslaunch mrs_uav_trajectory_loader start_tracking.launch config:=./config/trajectory_loader.yaml path:=`rospack find mrs_uav_controllers_cac`/trajectories
'
...
I  $TMUX_BIN -2 attach-session -t $SESSION_NAME
  exit
fi
