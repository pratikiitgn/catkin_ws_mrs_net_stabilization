...
#include <pratik_controller_one_link/controller_one_linkConfig.h>
...
namespace pratik_controller_one_link
{

namespace controller_one_link
{
...
typedef struct
{
  ...
  double kq_1;          // cable attitude gains
  double kq_2;          // cable attitude gains
  double kq_3;          // cable attitude gains
  double kq_dot_1;      // cable attitude gains
  double kq_dot_2;      // cable attitude gains
  double kq_dot_3;      // cable attitude gains
} Gains_t;

/* //{ class ControllerOneLink */

class ControllerOneLink : public mrs_uav_managers::Controller {
...
private:
  ...
  typedef pratik_controller_one_link::controller_one_linkConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>            Drs_t;
  boost::shared_ptr<Drs_t>                                    drs_;
  void                                                        callbackDrs(pratik_controller_one_link::controller_one_linkConfig& config, uint32_t level);
...
bool ControllerOneLink::initialize(const ros::NodeHandle& nh, std::shared_ptr<mrs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                                   std::shared_ptr<mrs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  success *= private_handlers->loadConfigFile(ros::package::getPath("pratik_controller_one_link") + "/config/controller_one_link.yaml");

  if (!success) {
    return false;
  }

  mrs_lib::ParamLoader param_loader(nh_, "ControllerOneLink");

  const std::string yaml_namespace = "pratik_controller_one_link/default_gains/";
  ...
  // gain filtering
  const std::string yaml_namespace_gain = "pratik_controller_one_link/gain_filtering/";
  ...
  const std::string yaml_namespace_ang_rate_ff = "pratik_controller_one_link/angular_rate_feedforward/";
  ...
  // constraints
  private_handlers->param_loader->loadParam("pratik_controller_one_link/constraints/tilt_angle_failsafe/enabled", _tilt_angle_failsafe_enabled_);
  private_handlers->param_loader->loadParam("pratik_controller_one_link/constraints/tilt_angle_failsafe/limit", _tilt_angle_failsafe_);
  ...
  if (_tilt_angle_failsafe_enabled_ && std::abs(_tilt_angle_failsafe_) < 1e-3) {
    ROS_ERROR("[ControllerOneLink]: constraints/tilt_angle_failsafe/enabled = 'TRUE' but the limit is too low");
    return false;
  }
  ...
  if (!param_loader.loadedSuccessfully()) {
    ROS_ERROR("[ControllerOneLink]: could not load all parameters!");
    return false;
  }
  ...
  Drs_t::CallbackType f = boost::bind(&ControllerOneLink::callbackDrs, this, _1, _2);
  drs_->setCallback(f);

  // | ------------------------- timers ------------------------- |

  timer_gains_ = nh_.createTimer(ros::Rate(_gain_filtering_rate_), &ControllerOneLink::timerGains, this, false, false);

  // | ------------------ initialize subscribers ----------------- |

  mrs_lib::SubscribeHandlerOptions shopts;
  shopts.nh                 = nh;
  shopts.node_name          = "ControllerOneLink";
  ...
  sh_cable_states           = mrs_lib::SubscribeHandler<nav_msgs::Odometry>(shopts, "/sim_one_link/uav1/cable_state",
                                                                                            &ControllerOneLink::callback_cable_states, this);
  ...
  ROS_INFO("[ControllerOneLink]: initialized");
...
bool ControllerOneLink::activate(const ControlOutput& last_control_output) {
  ...
  if (activation_control_output_.diagnostics.mass_estimator) {
    ...
    ROS_INFO("[ControllerOneLink]: setting mass difference from the last control output: %.2f kg", uav_mass_difference_);
  }
  ...
  ROS_INFO("[ControllerOneLink]: activated");
...
void ControllerOneLink::deactivate(void) {
  ...
  ROS_INFO("[ControllerOneLink]: deactivated");
}
...
void ControllerOneLink::updateInactive(const mrs_msgs::UavState& uav_state, [[maybe_unused]] const std::optional<mrs_msgs::TrackerCommand>& tracker_command) {
...
ControllerOneLink::ControlOutput ControllerOneLink::updateActive(const mrs_msgs::UavState& uav_state, const mrs_msgs::TrackerCommand& tracker_command) {
  ...
  if (fabs(dt) < 0.001) {
    ...
    ROS_DEBUG("[ControllerOneLink]: the last odometry message came too close (%.2f s)!", dt);
    dt = 0.01;
    // dt               = 0.004;
  }
  ...
  if (common_handlers_->control_output_modalities.attitude) {
    ROS_INFO_THROTTLE(1.0, "[ControllerOneLink]: desired attitude output modality is available");
  }
  ...
  if (common_handlers_->detailed_model_params) {
    ...
    // ROS_INFO_STREAM_THROTTLE(1.0, "[ControllerOneLink]: UAV inertia is: " << detailed_model_params.inertia);
  }
  ...
  Eigen::Array3d  kq_cable(0, 0, 0);
  Eigen::Array3d  kq_dot_cable(0, 0, 0);
  ...
  kq_cable(0) = gains.kq_1;
  kq_cable(1) = gains.kq_2;
  kq_cable(2) = gains.kq_3;

  kq_dot_cable(0) = gains.kq_dot_1;
  kq_dot_cable(1) = gains.kq_dot_2;
  kq_dot_cable(2) = gains.kq_dot_3;
  ...
  // | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
  // | ---------------- prepare for Cable Attitude Controller --------------- |
  ...
  u_cable_input     = kq_cable * e_q.array()  + kq_dot_cable * e_q_dot.array();

  // | -------------------------------------------------------------------------------------------------------------------------------------------------------- |

  // | ---------------- prepare the final control output --------------- |
  Eigen::Vector3d u_control_input   = u_quad_input + u_cable_input;

  if (u_control_input(2) < 0) {
    ROS_WARN_THROTTLE(1.0, "[ControllerOneLink]: the calculated downwards desired force is negative (%.2f) -> mitigating flip", u_control_input(2));
  ...
  if (tracker_command.use_orientation) {
    ...
    if (tracker_command.use_heading) {
      try {
        ...
        ROS_ERROR_THROTTLE(1.0, "[ControllerOneLink]: could not set the desired heading");
      }
    }
    ...
    if (tracker_command.use_heading) {
      ...
      ROS_WARN_THROTTLE(10.0, "[ControllerOneLink]: desired heading was not specified, using current heading instead!");
  ...
  else
  {
    if (desired_thrust_force >= 0) {
      ...
      ROS_WARN_THROTTLE(1.0, "[ControllerOneLink]: just so you know, the desired throttle force is negative (%.2f)", desired_thrust_force);
    }
  }
  ...
  last_control_output_.diagnostics.controller       = "ControllerOneLink";
...
Eigen::Vector3d ControllerOneLink::orientationError(const Eigen::Matrix3d& R, const Eigen::Matrix3d& Rd) {
...
const mrs_msgs::ControllerStatus ControllerOneLink::getStatus() {
...
void ControllerOneLink::switchOdometrySource([[maybe_unused]] const mrs_msgs::UavState& new_uav_state) {
}
...
void ControllerOneLink::resetDisturbanceEstimators(void) {
}
...
const mrs_msgs::DynamicsConstraintsSrvResponse::ConstPtr ControllerOneLink::setConstraints([
  ...
  ROS_INFO("[ControllerOneLink]: updating constraints");
...
void ControllerOneLink::callbackDrs(pratik_controller_one_link::controller_one_linkConfig& config, [[maybe_unused]] uint32_t level) {

  mrs_lib::set_mutexed(mutex_drs_params_, config, drs_params_);

  ROS_INFO("[ControllerOneLink]: dynamic reconfigure params updated");
}
...
void ControllerOneLink::timerGains(const ros::TimerEvent& event) {

  mrs_lib::Routine    profiler_routine = profiler_.createRoutine("timerGains", _gain_filtering_rate_, 1.0, event);
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer("ControllerOneLink::timerGains", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
  ...
  if (updated) {
    ...
    ROS_INFO_THROTTLE(10.0, "[ControllerOneLink]: gains have been updated");
  }
}
...
std::optional<mrs_msgs::HwApiAttitudeRateCmd> ControllerOneLink::attitudeController(const mrs_msgs::UavState& uav_state, const mrs_msgs::HwApiAttitudeCmd& reference,
                                                                 const Eigen::Vector3d& ff_rate, const Eigen::Vector3d& rate_saturation,
                                                                 const Eigen::Vector3d& gains, const bool& parasitic_heading_rate_compensation) {
...
void ControllerOneLink::callback_cable_states(const nav_msgs::Odometry::ConstPtr msg) {
...
float ControllerOneLink::clipping_angle(float max_value, float current_angle){
...
float ControllerOneLink::clipping_net_thrust_force(float max_value, float current_thrust){
...
Eigen::Vector3d ControllerOneLink::clipping_e_x_q(Eigen::Vector3d e_x_q_vector){
...
Eigen::Vector3d ControllerOneLink::clipping_e_x_q_dot(Eigen::Vector3d e_x_q_dot_vector){
...
float ControllerOneLink::distance_bt_two_pts(Eigen::Vector3d A, Eigen::Vector3d B){
...
Eigen::Vector3d ControllerOneLink::Matrix_vector_mul(Eigen::Matrix3d R, Eigen::Vector3d v){
  Eigen::Vector3d mul_vector (R(0,0)*v[0] + R(0,1)*v[1] + R(0,2)*v[2], R(1,0)*v[0] + R(1,1)*v[1] + R(1,2)*v[2],  R(2,0)*v[0] + R(2,1)*v[1] + R(2,2)*v[2]);
  return mul_vector;
}

Eigen::Vector3d ControllerOneLink::Rotation_matrix_to_Euler_angle(Eigen::Matrix3d R){
...
Eigen::Matrix3d ControllerOneLink::so3transform(const Eigen::Vector3d& body_z, const ::Eigen::Vector3d& heading, const bool& preserve_heading) {
...
double ControllerOneLink::calculateGainChange(const double dt, const double current_value, const double desired_value, const bool bypass_rate, std::string name,
                                          bool& updated) {
  ...
  if (std::abs(change) > 1e-3) {
    ROS_DEBUG("[ControllerOneLink]: changing gain '%s' from %.2f to %.2f", name.c_str(), current_value, desired_value);
    updated = true;
  }
...
double ControllerOneLink::getHeadingSafely(const mrs_msgs::UavState& uav_state, const mrs_msgs::TrackerCommand& tracker_command) {
...
// void ControllerOneLink::Publisher_QuadState(const mrs_msgs::UavState& uav_state){
...
// void ControllerOneLink::timerPublishQuadState([[maybe_unused]] const ros::TimerEvent& te, const mrs_msgs::UavState& uav_state) {
...
PLUGINLIB_EXPORT_CLASS(pratik_controller_one_link::controller_one_link::ControllerOneLink, mrs_uav_managers::Controller)
