...
#include <pratik_sim_mrs_net_free_ends/sim_mrs_net_free_endsConfig.h>
...
/* class PratikSimMrsNetFreeEnds //{ */

class PratikSimMrsNetFreeEnds : public nodelet::Nodelet {
...
private:
  ...
  typedef pratik_sim_mrs_net_free_ends::sim_mrs_net_free_endsConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>             Drs_t;
  boost::shared_ptr<Drs_t>                                     drs_;
  void                                                         callbackDrs(pratik_sim_mrs_net_free_ends::sim_mrs_net_free_endsConfig& config, uint32_t level);
...
void PratikSimMrsNetFreeEnds::onInit() {
  ...
  mrs_lib::ParamLoader param_loader(nh_, "PratikSimMrsNetFreeEnds");
  ...
  for (size_t i = 0; i < uav_names.size(); i++) {
    ...
    ROS_INFO("[PratikSimMrsNetFreeEnds]: initializing '%s'", uav_name.c_str());
  ...
  Drs_t::CallbackType f = boost::bind(&PratikSimMrsNetFreeEnds::callbackDrs, this, _1, _2);
  drs_->setCallback(f);

  if (!param_loader.loadedSuccessfully()) {
    ROS_ERROR("[PratikSimMrsNetFreeEnds]: could not load all parameters!");
    ros::shutdown();
  }
  ...
  timer_main_ = nh_.createWallTimer(ros::WallDuration(1.0 / (_simulation_rate_ * drs_params_.realtime_factor)), &PratikSimMrsNetFreeEnds::timerMain, this);

  timer_status_ = nh_.createWallTimer(ros::WallDuration(1.0), &PratikSimMrsNetFreeEnds::timerStatus, this);

  // | ----------------------- finish init ---------------------- |

  is_initialized_ = true;

  ROS_INFO("[PratikSimMrsNetFreeEnds]: initialized");
}
...
void PratikSimMrsNetFreeEnds::timerMain([[maybe_unused]] const ros::WallTimerEvent& event) {

  if (!is_initialized_) {
    return;
  }

  ROS_INFO_ONCE("[PratikSimMrsNetFreeEnds]: main timer spinning");
...
void PratikSimMrsNetFreeEnds::timerStatus([[maybe_unused]] const ros::WallTimerEvent& event) {
  ...
  ROS_INFO_THROTTLE(0.1, "[PratikSimMrsNetFreeEnds]: %s, desired RTF = %.2f, actual RTF = %.2f", drs_params.paused ? "paused" : "running",
...
void PratikSimMrsNetFreeEnds::callbackDrs(pratik_sim_mrs_net_free_ends::sim_mrs_net_free_endsConfig& config, [[maybe_unused]] uint32_t level) {
  ...
  ROS_INFO("[PratikSimMrsNetFreeEnds]: DRS updated params");
}
...
void PratikSimMrsNetFreeEnds::handleCollisions(void) {
...
void PratikSimMrsNetFreeEnds::publishPoses(void) {
...
PLUGINLIB_EXPORT_CLASS(pratik_sim_mrs_net_free_ends::PratikSimMrsNetFreeEnds, nodelet::Nodelet)
