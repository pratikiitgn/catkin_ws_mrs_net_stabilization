...
#include <pratik_sim_one_link/sim_one_linkConfig.h>
...
namespace pratik_sim_one_link
{

typedef std::vector<Eigen::VectorXd> my_vector_of_vectors_t;

/* class PratikSimOneLink //{ */

class PratikSimOneLink : public nodelet::Nodelet {
...
private:
  ...
  typedef pratik_sim_one_link::sim_one_linkConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>             Drs_t;
  boost::shared_ptr<Drs_t>                                     drs_;
  void                                                         callbackDrs(pratik_sim_one_link::sim_one_linkConfig& config, uint32_t level);
...
void PratikSimOneLink::onInit() {
  ...
  mrs_lib::ParamLoader param_loader(nh_, "PratikSimOneLink");
  ...
  for (size_t i = 0; i < uav_names.size(); i++) {
    ...
    ROS_INFO("[PratikSimOneLink]: initializing '%s'", uav_name.c_str());
  ...
  Drs_t::CallbackType f = boost::bind(&PratikSimOneLink::callbackDrs, this, _1, _2);
  drs_->setCallback(f);

  if (!param_loader.loadedSuccessfully()) {
    ROS_ERROR("[PratikSimOneLink]: could not load all parameters!");
    ros::shutdown();
  }
  ...
  timer_main_ = nh_.createWallTimer(ros::WallDuration(1.0 / (_simulation_rate_ * drs_params_.realtime_factor)), &PratikSimOneLink::timerMain, this);

  timer_status_ = nh_.createWallTimer(ros::WallDuration(1.0), &PratikSimOneLink::timerStatus, this);

  // | ----------------------- finish init ---------------------- |

  is_initialized_ = true;

  ROS_INFO("[PratikSimOneLink]: initialized");
}
...
void PratikSimOneLink::timerMain([[maybe_unused]] const ros::WallTimerEvent& event) {

  if (!is_initialized_) {
    return;
  }

  ROS_INFO_ONCE("[PratikSimOneLink]: main timer spinning");
...
void PratikSimOneLink::timerStatus([[maybe_unused]] const ros::WallTimerEvent& event) {
  ...
  ROS_INFO_THROTTLE(0.1, "[PratikSimOneLink]: %s, desired RTF = %.2f, actual RTF = %.2f", drs_params.paused ? "paused" : "running",
...
void PratikSimOneLink::callbackDrs(pratik_sim_one_link::sim_one_linkConfig& config, [[maybe_unused]] uint32_t level) {
  ...
  ROS_INFO("[PratikSimOneLink]: DRS updated params");
}
...
void PratikSimOneLink::handleCollisions(void) {
...
void PratikSimOneLink::publishPoses(void) {
  ...
  for (size_t i = 0; i < uavs_.size(); i++) {
    ...
    // ROS_INFO("Cable state from sim_one_link-> [%2.3f, %2.3f, %2.3f]",state.q_cable(0), state.q_cable(1), state.q_cable(2));
...
PLUGINLIB_EXPORT_CLASS(pratik_sim_one_link::PratikSimOneLink, nodelet::Nodelet)
