...
#include <pratik_sim_mrs_net_control_link_only/sim_mrs_net_control_link_onlyConfig.h>
...
/* class PratikSimMrsNetControlLinkOnly //{ */

class PratikSimMrsNetControlLinkOnly : public nodelet::Nodelet {
...
private:
  ...
  typedef pratik_sim_mrs_net_control_link_only::sim_mrs_net_control_link_onlyConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>             Drs_t;
  boost::shared_ptr<Drs_t>                                     drs_;
  void                                                         callbackDrs(pratik_sim_mrs_net_control_link_only::sim_mrs_net_control_link_onlyConfig& config, uint32_t level);
...
void PratikSimMrsNetControlLinkOnly::onInit() {
  ...
  mrs_lib::ParamLoader param_loader(nh_, "PratikSimMrsNetControlLinkOnly");
  ...
  for (size_t i = 0; i < uav_names.size(); i++) {
    ...
    ROS_INFO("[PratikSimMrsNetControlLinkOnly]: initializing '%s'", uav_name.c_str());
  ...
  Drs_t::CallbackType f = boost::bind(&PratikSimMrsNetControlLinkOnly::callbackDrs, this, _1, _2);
  drs_->setCallback(f);

  if (!param_loader.loadedSuccessfully()) {
    ROS_ERROR("[PratikSimMrsNetControlLinkOnly]: could not load all parameters!");
    ros::shutdown();
  }
  ...
  timer_main_ = nh_.createWallTimer(ros::WallDuration(1.0 / (_simulation_rate_ * drs_params_.realtime_factor)), &PratikSimMrsNetControlLinkOnly::timerMain, this);

  timer_status_ = nh_.createWallTimer(ros::WallDuration(1.0), &PratikSimMrsNetControlLinkOnly::timerStatus, this);

  // | ----------------------- finish init ---------------------- |

  is_initialized_ = true;

  ROS_INFO("[PratikSimMrsNetControlLinkOnly]: initialized");
}
...
void PratikSimMrsNetControlLinkOnly::timerMain([[maybe_unused]] const ros::WallTimerEvent& event) {

  if (!is_initialized_) {
    return;
  }

  ROS_INFO_ONCE("[PratikSimMrsNetControlLinkOnly]: main timer spinning");
...
void PratikSimMrsNetControlLinkOnly::timerStatus([[maybe_unused]] const ros::WallTimerEvent& event) {
  ...
  ROS_INFO_THROTTLE(0.1, "[PratikSimMrsNetControlLinkOnly]: %s, desired RTF = %.2f, actual RTF = %.2f", drs_params.paused ? "paused" : "running",
...
void PratikSimMrsNetControlLinkOnly::callbackDrs(pratik_sim_mrs_net_control_link_only::sim_mrs_net_control_link_onlyConfig& config, [[maybe_unused]] uint32_t level) {
  ...
  ROS_INFO("[PratikSimMrsNetControlLinkOnly]: DRS updated params");
}
...
void PratikSimMrsNetControlLinkOnly::handleCollisions(void) {
...
void PratikSimMrsNetControlLinkOnly::publishPoses(void) {
...
PLUGINLIB_EXPORT_CLASS(pratik_sim_mrs_net_control_link_only::PratikSimMrsNetControlLinkOnly, nodelet::Nodelet)
